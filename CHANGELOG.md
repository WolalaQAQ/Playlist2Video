...
- Removed browser preview audio URLs from the Remotion export props so the muted video-only render pass no longer downloads per-track audio.
- Added a regression test covering the video-only export path to prevent Remotion audio downloads from reappearing.
- Changed segmented Remotion chunk scheduling from fixed-size batch waits to a worker queue, so a freed chunk slot starts the next pending chunk immediately.
- Shared an in-flight Remotion server/composition startup across queued workers after transient chunk retries, preventing duplicate restarted bundle servers.

### Design Rationale

- Playlist2Video already concatenates and volume-adjusts playlist audio through FFmpeg before final muxing, so Remotion does not need `<Audio>` elements during MP4 export.
- Keeping the Remotion pass free of audio downloads avoids `Error while downloading ... audio ... cancelled` aborts during long segmented/concurrent exports without reducing GL rendering, chunk parallelism, or render concurrency.
- Queue-style chunk dispatch keeps the configured chunk parallelism saturated better than batch dispatch, especially when one chunk is slower than its peers.

### Notes & Caveats

- Browser preview playback is unchanged; preview projects still receive `audioPreviewUrl` and continue using Remotion audio in the in-app player.
- The final MP4 audio track remains generated by the existing FFmpeg concat and final stream-copy mux stages.
- Retriable chunk failures still wait for active chunks to finish before restarting the shared Remotion server and dispatching retry work.
